...
async function fetchUnreadNotificationData(buyerId) {
...
async function markTypeAsRead(buyerId, type) {
...
const NotificationManager = {
  ...
  async init(buyerId) {
    ...
    if (NotificationManager.cache.fetchInterval > 0) {
      NotificationManager.cache.timerId = setInterval(() => {
        NotificationManager.syncUnreadCounts(buyerId).catch(() => {});
  ...
  async syncUnreadCounts(buyerId) {
    if (!buyerId) return;
    if (NotificationManager.cache.isSyncing) return;

    NotificationManager.cache.isSyncing = true;
    try {
      const { unreadCount } = await fetchUnreadNotificationData(buyerId);
      NotificationManager.cache.unreadCounts.totalUnread = Number(unreadCount) || 0;
      NotificationManager.cache.unreadCounts.account = NotificationManager.cache.unreadCounts.totalUnread;
      updateNotificationBadge(NotificationManager.cache.unreadCounts.totalUnread);
    } finally {
...
function updateRefundBadge(buyerId) {
  if (window.skipNotificationRefundBadge) {
    return;
  }
...
>>>>>>> parent of e42effb (buyer and sellers otification)
}
