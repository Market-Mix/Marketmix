...
function createShopMore(product) {
  const container = document.getElementById('shop-more-section');
  if (!container) return;
  ...
  const html = `
    <div style="padding:16px;border:1px solid #e2e8f0;border-radius:12px;background:#fafafa">
      <h3 style="margin:0 0 16px;font-size:16px;font-weight:700;color:#1e293b">More from this seller</h3>
  ...
  // Add click handlers for seller products
